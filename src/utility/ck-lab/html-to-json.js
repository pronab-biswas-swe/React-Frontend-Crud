...
function htmlToPdfMake(htmlText, options) {
	...
	this.imagesByReference =
		options && typeof options.imagesByReference === 'boolean' ? options.imagesByReference : false;
	this.removeExtraBlanks =
		options && typeof options.removeExtraBlanks === 'boolean' ? options.removeExtraBlanks : false;
	this.showHidden = options && typeof options.showHidden === 'boolean' ? options.showHidden : false;
	this.removeTagClasses =
		options && typeof options.removeTagClasses === 'boolean' ? options.removeTagClasses : false;
	...
	this.fontSizes =
		options && Array.isArray(options.fontSizes) ? options.fontSizes : [10, 14, 16, 18, 20, 24, 28];
	...
	this.parseElement = function (element, parents) {
		...
		switch (element.nodeType) {
			...
			case 1: {
				// ELEMENT_NODE
				if (
					(!this.showHidden && element.style.display && element.style.display === 'none') ||
				...
				switch (nodeName) {
						...
						if (this.tableAutoSize) {
							...
							var fullWidth =
								...
								(element.getAttribute('style') || '')
									.replace(/width\s*:\s*100%/, 'width:100%')
									.includes('width:100%');
										...
										if (tableHaveWidth) {
											...
											var cellPercentage =
												cellWidth === 'auto'
													...
													: (cellWidth.toString().replace('%', '') * tableWidth) / 100;
							...
							if (tableWidths.length > 0) {
								...
								if (fullWidth)
									tableWidths = tableWidths.map(function (w) {
										return w === 'auto' ? '*' : w;
									});
					...
					case 'LI': {
						...
						if (ret.stack && !ret.stack[ret.stack.length - 1].text) {
							...
							ret = [Array.isArray(text) ? { stack: text } : { text: text }, ret.stack[ret.stack.length - 1]];
						}
	...
	this.searchForStack = function (ret) {
		if (Array.isArray(ret.text)) {
			for (var i = 0; i < ret.text.length; i++) {
				if (
					ret.text[i].stack ||
					['P', 'DIV', 'TABLE', 'SVG', 'UL', 'OL', 'IMG', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6'].indexOf(
						ret.text[i].nodeName
					) > -1
				)
					return true;
...
export default function (htmlText, options) {
